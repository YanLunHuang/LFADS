  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      if (e % CHECKPOINT == 0) {
        ...
        for(int i = 0; i < N_OUT_2; i++) {
          std::cout << pr[i] << " ";
        }
        std::cout << std::endl;
        std::cout << "Quantized predictions" << std::endl;
        //hls-fpga-machine-learning insert quantized
        nnet::print_result_switch<result_t, N_OUT_2,N_OUT_2>(layer2_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<result_t, N_OUT_2,N_OUT_2>(layer2_out, fout);
